...
{
	if(_hasBattery) {
		_emu->GetBatteryManager()->LoadBattery("Gameboy", IsPrimaryConsole() ? ".sav" : ".p2.sav", _cartRam, _cartRamSize);
	}
}
...
{
	if(_hasBattery) {
		_emu->GetBatteryManager()->SaveBattery("Gameboy", IsPrimaryConsole() ? ".sav" : ".p2.sav", _cartRam, _cartRamSize);
	}
	_cart->SaveBattery();
