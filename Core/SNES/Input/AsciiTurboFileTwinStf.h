...
public:
	...
	void Init() override
	{
		...
		_emu->GetBatteryManager()->LoadBattery("SNES", ".turbofile_stf.sav", _data, AsciiTurboFileTwinStf::FileSize);
	}

	void SaveBattery() override
	{
		_emu->GetBatteryManager()->SaveBattery("SNES", ".turbofile_stf.sav", _data, AsciiTurboFileTwinStf::FileSize);
	}
				...
				if(!_firstAccess) {
					if(_writeMode) {
						if(_didReadWithStrobe) {
							//Turbo File Twin ignores writes at out-of-range addresses
						...
						//Turbo File Twin ignores reads at out-of-range addresses
						if(_position < FileSize) {
