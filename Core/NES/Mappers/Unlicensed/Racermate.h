...
protected:
	...
	void LoadBattery() override
	{
		...
		if(_hasChrBattery && _saveChrRamSize > 0) {
			_emu->GetBatteryManager()->LoadBattery("NES", ".chr.sav", _chrRam + (_chrRamSize - _saveChrRamSize), _saveChrRamSize);
		} else if(_hasChrBattery) {
			_emu->GetBatteryManager()->LoadBattery("NES", ".chr.sav", _chrRam + (_chrRamSize / 2), _chrRamSize / 2);
		}
	}
	...
	void SaveBattery() override
	{
		...
		if(_hasChrBattery && _saveChrRamSize > 0) {
			_emu->GetBatteryManager()->SaveBattery("NES", ".chr.sav", _chrRam + (_chrRamSize - _saveChrRamSize), _saveChrRamSize);
		} else if(_hasChrBattery) {
			_emu->GetBatteryManager()->SaveBattery("NES", ".chr.sav", _chrRam + (_chrRamSize / 2), _chrRamSize / 2);
		}
	}
};